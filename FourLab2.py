...
class VidCreation(GraphScene):
    ...
    def construct(self):
        ...
        frequency_axes = Fourier.get_frequency_axes(self)
        func =lambda t: 3*np.cos(2*PI*1*t)+0.01*np.cos(2*np.pi*9*t)
        time_func = time_axes.get_graph(func,color = YELLOW)
        t_sig = VGroup(time_axes,time_func).to_edge(LEFT,buff = 0)
        result = Fourier.get_fourier_graph(self,self.frequency_axes, func, 0, 15)
        f_sig = VGroup(frequency_axes,result)
        self.play(ShowCreation(time_func),run_time = 3)
        self.wait()
